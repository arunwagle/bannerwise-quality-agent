...
# DBTITLE 1,Header
# MAGIC %md
# MAGIC # Create System Tables
# MAGIC
# MAGIC Creates (or recreates) the Bannerwise Quality Agent system tables and seeds the certified QA corpus.
# MAGIC
# MAGIC **Tables:** `certified_qa_corpus` (CDF enabled), `certified_qa_corpus_draft`, `query_history`, `sme_review_queue`
# MAGIC
# MAGIC **Behavior:** Uses `CREATE OR REPLACE TABLE` — always applies the latest schema. Safe to re-run.
...
spark.sql(f"""
CREATE OR REPLACE TABLE {catalog_name}.{schema_name}.certified_qa_corpus (
    id                  STRING        NOT NULL  COMMENT 'Unique corpus entry identifier (e.g. QA-0001)',
    question            STRING        NOT NULL  COMMENT 'Certified question text (with {{param}} placeholders for display/judge)',
    embedding_text      STRING        NOT NULL  COMMENT 'Embedding-optimized text (params stripped for better VS retrieval)',
    question_embedding  ARRAY<FLOAT>            COMMENT 'Vector embedding of embedding_text (populated by VS sync)',
...
spark.sql(f"""
CREATE OR REPLACE TABLE {catalog_name}.{schema_name}.certified_qa_corpus_draft (
...
# DBTITLE 1,Create query_history
spark.sql(f"""
CREATE OR REPLACE TABLE {catalog_name}.{schema_name}.query_history (
...
# DBTITLE 1,Create sme_review_queue
spark.sql(f"""
CREATE OR REPLACE TABLE {catalog_name}.{schema_name}.sme_review_queue (
...
# DBTITLE 1,Summary
print(f"\nAll system tables created in {catalog_name}.{schema_name}")
